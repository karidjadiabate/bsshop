...
class Database{
    ...
    public function getAllRecords1($tableName,$tableName1, $fields='*',$key,$keyStrange, $cond='', $orderBy='', $limit='')
    {
        $stmt = $this->pdo->prepare("SELECT $fields FROM $tableName INNER JOIN $tableName1 ON $key=$keyStrange  WHERE 1 ".$cond." ".$orderBy." ".$limit);
        $stmt->execute();
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        return $rows;
    }
    public function getAllRecords3($tableName,$tableName1,$tableName2, $fields='*',$key,$keyStrange,$keyStrange1,$otherkey, $cond='', $orderBy='', $limit='')
    {
        $stmt = $this->pdo->prepare("SELECT $fields FROM $tableName INNER JOIN $tableName1 ON $key=$keyStrange  INNER JOIN $tableName2 ON $otherkey=$keyStrange1  WHERE 1 ".$cond." ".$orderBy." ".$limit);
        $stmt->execute();
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        return $rows;
    }
    ...
    public function getQueryCountNb($tableName, $field, $cond='')
    {
        $stmt = $this->pdo->prepare("SELECT count($field) as total FROM $tableName WHERE 1 ".$cond);
        try {
            $stmt->execute();
