...
$condition .= ' AND orders.statut="V"';
$allCmdEAV = $db->getAllRecords1('orders', 'orders_details', '*,orders.created_at as orders_created_at,GROUP_CONCAT( orders_details.price SEPARATOR "," ) as priceInOrder,GROUP_CONCAT( orders_details.quantity SEPARATOR "," ) as allQuantity,SUM(price) as allPrice,GROUP_CONCAT( orders_details.products_id SEPARATOR "," ) as AllProducts', 'id', 'orders_id', $condition, 'GROUP BY orders.id,orders.created_at ORDER BY orders.created_at DESC');
...
<body class="navbar-fixed sidebar-fixed" id="body">
          ...
          <div class="row">
            <div class="col-xl-12">
              <?php if (isset($_REQUEST['msg']) and $_REQUEST['msg'] == "rds") {
                ...
                echo '<script type="text/javascript">swal({ title: "Accepté !", text: "Commande livrée avec succès!", icon: "success", confirmButtonText: "OK" });</script>';
              ...
              <div class="card card-default">
                <form class="mt-5 ml-8 " method="GET">
                  ...
                  <div class="form-row">
                    ...
                    <div class="col-4">
                      ...
                      <a href="<?php echo PATH ?>commandes/valides.php" class="btn btn-danger btn-sm ">Reinitialiser</a>
                ...
                <div class="card-header align-items-center">
                  <h2 class="">LISTE DES COMMANDES VALIDEES </h2>
                        ...
                        <table class="display compact cell-border nowrap" cellspacing="0" width="100%" id='maintable' class="table table-hover table-product" style="width:100%">
                          <thead class="text-center ">
                            <tr>
                              ...
                              <th>Adresse <br>De livraison</th>
                              <th>Prix <br>de livraison</th>
                            ...
                            if (count($allCmdEAV) > 0) {
                              foreach ($allCmdEAV as $val) {
                                ...
                                <tr>
                                  ...
                                  ?>
                                  <td><?php if ($val['allPrice'] <= 10) {
                                        $prixDeLivraison = 1000;
                                      } else {
                                        $prixDeLivraison = $val['allPrice'] * 0.1;
                                      };
                                      echo $prixDeLivraison ?></td>
                                  <td><?php echo $val['allPrice'] + $prixDeLivraison ?></td>
                                  <td><?php echo $val['orders_created_at'] ?></td>
                                  <td class="text-sm"><span class="bg-dark text-white p-3 font-weight-bold ">COMMANDE VALIDEE</span></td>
                                  <td>
                                    <a class="btn btn-success btn-sm" href="#" onClick="update('<?php echo $val['id']; ?>')">Livrer</a>
                                    <!-- <a class="btn btn-primary btn-sm" href="printRecu.php?orders_id=<?php //echo $val['id']; 
                                                                                                          ?>"target="_blank">Reçu</a> -->
                                    <a class="btn btn-warning btn-sm" onclick="Impression('<?php echo $val['id'] ?>')">Reçu </a>
                                    <!-- <a class="btn btn-warning btn-sm" href="../../tcpdf/source/printrecu.php?orders_id=<?php echo $val['id']; ?>" target="_blank" onclick='Impression(<?php echo $val["id"] ?>)'>Reçu </a> -->
                              ...
                              <tr>
                                <td colspan="14"><b>Aucun enregistrement trouvé</b></td>
                              </tr>
                            <?php } ?>
                          </tbody>
                        </table>
  ...
  <script>
    ...
    function update(id) {
      swal({
          title: "Confirmation",
          text: "Etes-vous sûr que cette commande a été livrée ?",
          icon: "warning",
          buttons: ["Non, annuler", "Oui, je confirme"],
          dangerMode: true,
        })
        .then((willDelete) => {
          if (willDelete) {
            window.location.href = "supprimer.php?delId=" + id + "&sec=V";
          } else {
            swal({
              title: "Alerte !",
              text: "Operation de livraison non confirmée!",
              icon: "warning",
              confirmButtonText: "OK"
            });
          }
