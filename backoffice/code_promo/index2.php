...
define('MENU', 'TYPECODEPROMO');
$condition = '';
$condition .= 'AND statut="1"';
$allPrices = $db->getAllRecords('type_code_promos', '*', $condition, 'ORDER BY name ASC');
...
if (isset($_REQUEST['valider']) and $_REQUEST['valider'] != "") {
  ...
  $checkCat  =   $db->getAllRecords('type_code_promos', '*', 'AND statut="1"');
  ...
  $error = '';
  if ($cpt > 0) {
    header("location:index2.php?msg=exist");
    exit;
  } else {
    $data   =   array(
      'name' => $libelleI,
    ...
    $insert =   $db->insert('type_code_promos', $data);
    if ($insert) {
      header('location:index2.php?msg=ras');
      exit;
    } else {
      header('location:index2.php?msg=rna');
      exit;
    }
  }
}
...
if (isset($_REQUEST['update']) and $_REQUEST['update'] != "") {
  ...
  $insert =   $db->update('type_code_promos', $data, array('id' => $category_Id));
  if ($insert) {

    header('location:index2.php?msg=rus');
    exit;
  } else {
            ...
            <div class="col-xl-12">
              ...
              <div class="card card-default">
                <div class="card-header align-items-center">
                  <h2 class="">LISTE DES TYPE DE CODE PROMO SUR LES COMMANDES</h2>
                            ...
                            if (count($allPrices) > 0) {
                              foreach ($allPrices as $val) {
                                ...
                                <tr>
                                  ...
                                  <td>
                                  <?php
                                        $address = explode(' ', $val['description']);
                                        $ff = 0;
                    ...
                    <div class="row">
                      <div class="col-lg-12">
                        ...
                        <div class="form-group mb-5">
                          <label for="new-product">Libelle du type de code promo<span class="text-danger">*</span></label>
                          <input type="text" class="form-control" required id="new-product" name='libelleI' placeholder="ajouter un libelle">
                        </div>
                        <div class="form-group mb-5">
                          <label for="new-product">Description<span class="text-danger">*</span></label>
                          <textarea name="description" class="form-control" id="" cols="30"  required placeholder="A quoi sert ce type de code promo" rows="10"></textarea>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="modal-body p3 p-md-5">
                    <div class="row">
                      <div class="col-lg-8">
                        ...
                        <div class="form-group mb-5">
                          <label for="new-product">Description<span class="text-danger">*</span></label>
                          <textarea class="form-control" id="descriptionU" cols="30" name="descriptionU" required placeholder="A quoi sert ce type de code promo" rows="10"></textarea>
                        </div>
                      </div>
                    </div>
                  </div>
  ...
  <script>
              ...
              $.ajax({
                ...
                success: function(data) {
                  if (data['result'] == "deleteAllOk") {
                    window.location.href = "index2.php?msg=rds";
                  }
                }
              });
            } else {
    ...
    function supprimer(id) {
      swal({
        ...
        .then((willDelete) => {
          if (willDelete) {
            window.location.href = "supprimer.php?delId=" + id + "&sec=PRIX";
          } else {
            swal({
              title: "Alerte !",
              text: "Operation de suppression non confirmée!",
