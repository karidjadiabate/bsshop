...
$allCategories = $db->getAllRecords('categories', '*', $condition, 'ORDER BY name ASC');
$imageCatNames = $db->getAllRecords('images_category', '*');
...
if (isset($_REQUEST['valider']) and $_REQUEST['valider'] != "") {
  ...
  if (isset($_FILES['photo']) && !empty($_FILES['photo']['name'])) {
    ...
    if (in_array($extensionupload, $extensionok)) {
      ...
      $chemin = "../images/categories/" . $_FILES['photo']['name'];
      $deplacement = move_uploaded_file($_FILES['photo']['tmp_name'], $chemin);
    }
    $photo = $_FILES['photo']['name'];
  }
  $checkCat  =   $db->getAllRecords('categories', '*', 'AND statut="1"');
  ...
  $error = '';
  if ($cpt > 0) {
    header("location:categories.php?msg=exist");
    exit;
  } else {
    $data   =   array(
      'name' => $libelleI,
    ...
    if ($insert) {
      ...
      $insert3 =   $db->insert('images_category', $data2);
      if ($insert3) {
        header('location:categories.php?msg=ras');
        exit;
      } else {
...
if (isset($_REQUEST['update']) and $_REQUEST['update'] != "") {
  ...
  if (isset($_FILES['photoU']) && !empty($_FILES['photoU']['name'])) {
    ...
    if (in_array($extensionupload, $extensionok)) {
      ...
      $chemin = "../images/categories/" . $_FILES['photoU']['name'];
...
// var_dump( $photo);exit;
  ...
  $insert =   $db->update('categories', $data, array('id' => $category_Id));
  ...
  if ($insert) {
    $insert3 =   $db->update('images_category', $data, array('category_id' => $category_Id));
    if ($insert3) {
            ...
            <div class="col-xl-12">
              ...
              <div class="card card-default">
                <div class="card-header align-items-center">
                  <h2 class="">LISTE DES CATEGORIES DE PRODUITS </h2>
                  <a href="#" class="btn btn-primary" data-toggle="modal" data-target="#modal-stock"style="font-size:11px" >AJOUTER</a>
                </div>
                <div align="center">
                  <h3><b><?php echo count($allCategories); ?> enregistrement(s) trouvé(s)</b></h3>
                </div>
                <!-- Table Product -->
                <div class="row">
                  <div class="col-12">
                            ...
                            if (count($allCategories) > 0) {
                              foreach ($allCategories as $val) {
                                ...
                                <tr>
                                  ...
                                  <td class="py-0 xs">
                                    <?php foreach ($imageCatNames as $imageCatName) {
                                      if ($imageCatName['category_id'] == $val['id']) { ?>
                                        <img src="../images/categories/<?php echo $imageCatName['name'] ?>" height="110px" width="100px" alt="Product Image">
                                    <?php    }
                                    } ?>
                                  </td>
                              ...
                              <tr>
                                <td colspan="4"><b>Aucun enregistrement trouvé</b></td>
                              </tr>
                            <?php } ?>
                          </tbody>
                        </table>
              ...
              <form action="" method="POST" enctype="multipart/form-data">
                <div class="modal-content">
                  <div class="modal-header align-items-center p3 p-md-5">
                    <h2 class="modal-title" id="exampleModalGridTitle">Ajouter une categorie d'article</h2>
                    <div>
                      <button type="button" class="btn btn-light btn-pill mr-1 mr-md-2" data-dismiss="modal"> Retour </button>
                      <button type="submit" class="btn btn-primary  btn-pill" name="valider" style="background-color:black" value="valider"> Valider </button>
                    </div>
                    ...
                    <div class="row">
                      <div class="col-lg-8">
                        ...
                        <div class="form-group mb-5">
                          <label for="new-product">Libelle de la categorie <span class="text-danger">*</span></label>
                          <input type="text" class="form-control" required id="new-product" name='libelleI' placeholder="Ajouter un libelle">
                        </div>
                      </div>
                      <div class="col-lg-4">
          ...
          <div class="modal fade update-modal-stock" id="update-modal-stock" aria-labelledby="update-modal-stock" aria-hidden="true">
            <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
              <form action="" method="POST" enctype="multipart/form-data">
                <div class="modal-content">
                  <div class="modal-header align-items-center p3 p-md-5">
                    <h2 class="modal-title" id="exampleModalGridTitle">Modifier une categorie d'article</h2>
                    <div>
                      <button type="button" class="btn btn-light btn-pill mr-1 mr-md-2" data-dismiss="modal"> Retour </button>
  ...
  <script>
              ...
              $.ajax({
                ...
                success: function(data) {
                  if (data['result'] == "deleteAllOk") {
                    window.location.href = "categories.php?msg=rds";
                  }
                }
              });
            } else {
    ...
    function supprimer(id) {
      swal({
        ...
        .then((willDelete) => {
          if (willDelete) {
            window.location.href = "supprimer.php?delId=" + id + "&sec=CATEGORIE";
          } else {
            swal({
              title: "Alerte !",
              text: "Operation de suppression non confirmée!",
