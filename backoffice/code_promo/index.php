...
define('MENU', 'CODEPROMO');
$condition = '';
$condition .= 'AND statut="1"';
$allPrices = $db->getAllRecords('code_promo', '*', $condition, 'ORDER BY create_at ASC');
$typeCodePromo = $db->getAllRecords('type_code_promos', '*', $condition, 'ORDER BY name ASC');
...
if (isset($_REQUEST['valider']) and $_REQUEST['valider'] != "") {
  ...
  $checkCat  =   $db->getAllRecords('code_promo', '*', 'AND statut="1"');
  foreach ($checkCat as $key) {
    if (strtolower($key['code']) == strtolower($code)) {
      $cpt += 1;
    }
  }
  $error = '';
  if ($cpt > 0) {
    ...
    $data   =   array(
      'code' => $code,
      'id_type_code_promo' => $id_type_code_promo,
      'discount' => $discount,
      'max_usage' => $max_usage,
      'validity' => $validity,
      'valid' => 1,

      'create_by' => $_SESSION['userYopci']['nom'] . "#" . $_SESSION['userYopci']['iduser'],
    );
    $insert =   $db->insert('code_promo', $data);
    if ($insert) {
...
if (isset($_REQUEST['update']) and $_REQUEST['update'] != "") {
  ...
  $insert =   $db->update('code_promo', $data, array('id' => $category_Id));
  if ($insert) {
            ...
            <div class="col-xl-12">
              ...
              <div class="card card-default">
                <div class="card-header align-items-center">
                  <h2 class="">LISTE DES CODES PROMO SUR LES COMMANDES</h2>
                  <a href="#" class="btn btn-primary" data-toggle="modal" data-target="#modal-stock" style="font-size:11px">AJOUTER</a>
                </div>
                <div align="center">
                  <h3><b><?php echo count($allPrices); ?> enregistrement(s) trouvé(s)</b></h3>
                </div>
                <!-- Table Product -->
                <div class="row">
                  <div class="col-12">
                        ...
                        <table class="display compact cell-border nowrap" cellspacing="0" width="100%" id='maintable' class="table table-hover table-product" style="width:100%">
                          <thead class="text-center">
                            <tr>
                              ...
                              <th>Code</th>
                              <th>remise</th>
                              <th>Nombre d'usage<br> restants</th>
                              <th>Validité</th>
                              <th>Valide</th>
                              <th>Action</th>
                            </tr>
                            ...
                            if (count($allPrices) > 0) {
                              foreach ($allPrices as $val) {
                                ...
                                <tr>
                                  ...
                                  <td><?php echo $val['max_usage'] ?></td>
                                  <td><?php echo $val['validity'] ?></td>
                                  <td><?php if($val['valid']==1 && $val['validity']>date('Y-m-d')){echo '<span class="bg-success p-lg-1 text-white">valide</span>' ;}else{ echo '<span class="bg-danger text-white p-lg-1">expiré</span>';} ?></td>
                                  <td>
              ...
              <form action="" method="POST" enctype="multipart/form-data">
                <div class="modal-content">
                  <div class="modal-header align-items-center ">
                    <h2 class="modal-title" id="exampleModalGridTitle">Ajouter un prix de vente des articles</h2>
                    <div>
                    ...
                    <h3 class="h5 mb-5">les champs contenant <span class="text-danger">*</span> sont obligatoires</h3>
                      ...
                      <div class="col-lg-6">
                        <div class="form-group mb-5">
                          <label for="new-product">Libelle du code promo<span class="text-danger">*</span></label>
                          <input type="text" class="form-control" required  name='code' placeholder="ajouter un libelle">
                        </div>
                      </div>
                      <div class="col-lg-6">
                        <div class="form-group mb-5">
                          <label for="new-product">Nombre maximum d'utilisation<span class="text-danger">*</span></label>
                          <input type="number" min="1" class="form-control" required  name='max_usage'>
                        </div>
                      </div>
                    </div>
                    <div class="row">
                      <div class="col-lg-4">
                        <div class="form-group mb-5">
                          <label for="new-product">Remise<span class="text-danger">*</span></label>
                          <input type="text" class="form-control" required  name='discount' placeholder="ajouter une remise">
                        </div>
                      </div>
                      <div class="col-lg-4">
                        <div class="form-group mb-5">
                          <label for="new-product">Validité<span class="text-danger">*</span></label>
                          <input type="date" class="form-control" required  name="validity">
                        </div>
                      </div>
                      <div class="col-lg-4">
                      <label for="new-product">Type de code promo<span class="text-danger">*</span></label>
                        <select required name="id_type_code_promo" class="form-control">
                          <?php foreach ($typeCodePromo as $val) { ?>
              ...
              <form action="" method="POST" enctype="multipart/form-data">
                <div class="modal-content">
                  <div class="modal-header align-items-center p3 p-md-5">
                    <h2 class="modal-title" id="exampleModalGridTitle">Modifier un prix de vente des articles</h2>
                    <div>
                      <button type="button" class="btn btn-light btn-pill mr-1 mr-md-2" data-dismiss="modal"> Retour </button>
                  ...
                  <div class="modal-body ">
                    ...
                    <h3 class="h5 mb-5">les champs contenant <span class="text-danger">*</span> sont obligatoires</h3>
                      ...
                      <div class="col-lg-4">
                        <div class="form-group mb-5">
                          <label for="new-product">Libelle du code promo<span class="text-danger">*</span></label>
                          <input type="text" class="form-control" required id="code" name='code' placeholder="ajouter un libelle">
                        </div>
                      </div>
                      <div class="col-lg-4">
                        <div class="form-group mb-5">
                          <label for="new-product">Nombre maximum d'utilisation<span class="text-danger">*</span></label>
                          <input type="number" min="1" class="form-control" required id="max_usage" name='max_usage'>
                        </div>
                      </div>
                      <div class="col-lg-4">
                        <div class="form-group mb-5">
                          <label for="new-product">Remise<span class="text-danger">*</span></label>
                          <input type="text" class="form-control" required id="discount" name='discount' placeholder="ajouter une remise">
                        </div>
                      </div>
                    </div>
                    <div class="row">
                      <div class="col-lg-4">
                        <div class="form-group mb-5">
                          <label for="new-product">Statut<span class="text-danger">*</span></label>
                      ...
                      <div class="col-lg-4">
                        <div class="form-group mb-5">
                          <label for="new-product">Validité<span class="text-danger">*</span></label>
                          <input type="date" class="form-control" required id="validity" name="validity">
                        </div>
                      </div>
                      <div class="col-lg-4">
                      <label for="new-product">Type de code promo<span class="text-danger">*</span></label>
                        <select required name="id_type_code_promo" id="id_type_code_promo" class="form-control">
                          <?php foreach ($typeCodePromo as $val) { ?>
                            <option value="<?php echo $val['id']; ?>"><?php echo $val['name']; ?></option>
                          <?php } ?>
                        </select>
  ...
  <script>
    ...
    function supprimer(id) {
      swal({
        ...
        .then((willDelete) => {
          if (willDelete) {
            window.location.href = "supprimer.php?delId=" + id + "&sec=PRIX1";
          } else {
            swal({
              title: "Alerte !",
              text: "Operation de suppression non confirmée!",
    ...
    function update(id) {
      $.ajax({
        ...
        success: function(data) {
          console.log(data);
          $("#code").val(data[0]);
          $("#id_type_code_promo").val(data[1]);
          $("#max_usage").val(data[2]);
          $("#discount").val(data[3]);
          $("#validity").val(data[4]);
          $("#statut").val(data[5]);
          $("#category_Id").val(id);
