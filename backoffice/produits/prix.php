...
$allPrices = $db->getAllRecords('prix', '*', $condition, 'ORDER BY valeur ASC');
...
if (isset($_REQUEST['valider']) and $_REQUEST['valider'] != "") {
  ...
  $checkCat  =   $db->getAllRecords('prix', '*', 'AND statut="1"');
  foreach ($checkCat as $key) {
    if (strtolower($key['valeur']) == strtolower($libelleI)) {
      $cpt += 1;
    }
  }
  $error = '';
  if ($cpt > 0) {
    header("location:prix.php?msg=exist");
    exit;
  } else {
    $data   =   array(
      'valeur' => $libelleI,
    ...
    $insert =   $db->insert('prix', $data);
    if ($insert) {
        header('location:prix.php?msg=ras');
        exit;
      } else {
        header('location:prix.php?msg=rna');
        exit;
      }
  }
}
...
// var_dump($_REQUEST);exit;
  ...
  $insert =   $db->update('prix', $data, array('id' => $category_Id));
  if ($insert) {
   
      header('location:prix.php?msg=rus');
      exit;
    } else {
            ...
            <div class="col-xl-12">
              ...
              <div class="card card-default">
                <div class="card-header align-items-center">
                  <h2 class="">LISTE DES PRIX DE PRODUITS </h2>
                    ...
                    <div class="row">
                      <div class="col-lg-12">
                        ...
                        <div class="form-group mb-5">
                          <label for="new-product">Prix de vente de l'article<span class="text-danger">*</span></label>
                          <input type="number"min="500" step="50" class="form-control" required id="new-product" name='libelleI' placeholder="ajouter un prix">
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="modal-body p3 p-md-5">
                    <div class="row">
                      <div class="col-lg-8">
                        ...
                        <div class="form-group mb-5">
                          <label for="new-product">Prix de vente de l'article <span class="text-danger">*</span></label>
                          <input type="number"min="500" step="50" class="form-control" required id="LibelleU" name='LibelleU' placeholder="ajouter un prix">
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              $.ajax({
                ...
                success: function(data) {
                  if (data['result'] == "deleteAllOk") {
                    window.location.href = "prix.php?msg=rds";
                  }
                }
              });
            } else {
