  ...
  if (isset($_FILES['photo']) && !empty($_FILES['photo']['name'])) {
    ...
    if ($cpt > 0) {
      ...
      if ($insert) {
        ...
        for ($ph = 0; $ph < count($_FILES['photo']['type']); $ph++) {
          // $taillemax=2097152;
          $extensionok = ['jpg', 'jpeg', 'png', 'jfif'];
          // if($_FILES['photo']['size']<=$taillemax){
          $extensionupload = strtolower(substr(strrchr($_FILES['photo']['name'][$ph], '.'), 1));
          if (in_array($extensionupload, $extensionok)) {

            $chemin = "../images/products/" . $_FILES['photo']['name'][$ph];
            $deplacement = move_uploaded_file($_FILES['photo']['tmp_name'][$ph], $chemin);
          }
          $photo = $_FILES['photo']['name'][$ph];
...
if (isset($_REQUEST['update']) and $_REQUEST['update'] != "") {
  ...
  if (isset($_FILES['photoU']) && !empty($_FILES['photoU']['name'])) {
    // $taillemax=2097152;
    $db->update('images', array('statut' => 0), array('product_id' => $products_id));
    for ($ph = 0; $ph < count($_FILES['photoU']['type']); $ph++) {
      $extensionok = ['jpg', 'jpeg', 'png', 'jfif'];
      // if($_FILES['photo']['size']<=$taillemax){
      $extensionupload = strtolower(substr(strrchr($_FILES['photoU']['name'][$ph], '.'), 1));
      if (in_array($extensionupload, $extensionok)) {

        $chemin = "../images/products/" . $_FILES['photoU']['name'][$ph];
        $deplacement = move_uploaded_file($_FILES['photoU']['tmp_name'][$ph], $chemin);
      }
      $photoU = $_FILES['photoU']['name'][$ph];
      $data   =   array(
                            ...
                            if (count($allProducts) > 0) {
                              foreach ($allProducts as $val) {
                                ...
                                <tr>
                                  ...
                                  <td class="py-0 xs">
                                    <?php $RR = 0;
                                    foreach ($imageNames as $imageCatName) {
                    ...
                    <div class="row">
                      ...
                      <div class="col-lg-4">
                        <div class="custom-file">
                          <input type="file" accept=".jpg,.jfif,.jpeg,.png" required class="custom-file-input" name='photo[]' multiple id="customFile" placeholder="please imgae here">
          ...
          <div class="modal fade update-modal-stock" id="update-modal-stock" aria-labelledby="update-modal-stock" aria-hidden="true">
            <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
              <form action="" method="POST" enctype="multipart/form-data">
                <div class="modal-content">
                  <div class="modal-header align-items-center p3 p-md-5">
                    <h2 class="modal-title" id="exampleModalGridTitle">Modifier un article</h2>
                    <div>
                      <button type="button" class="btn btn-light btn-pill mr-1 mr-md-2" data-dismiss="modal"> Retour </button>
                    ...
                    <div class="row">
                      ...
                      <div class="col-lg-4" align="center">
                        ...
                        <div class="custom-file">
                          ...
                          <input type="file" class="custom-file-input" id="customFile" name='photoU[]' multiple accept=".jpg,.jfif,.jpeg,.png" placeholder="image ici">
                          <hr>
                        </div>
                      </div>
                    </div>
