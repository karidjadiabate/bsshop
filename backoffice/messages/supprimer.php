...
if (isset($_REQUEST['delId']) and $_REQUEST['delId'] != "" and isset($_REQUEST['sec']) and $_REQUEST['sec'] != "") {
    ...
    if($_REQUEST['sec']=="CATEGORIE"){
        ...
        }else if($_REQUEST['sec']=="DELMASSE"){
            $delete =   $db->update('comment_by_product', $data, array('id' => $editId));
            if ($delete) {
                    header('location:commentaires.php?msg=rds');
                    exit;
                } else {
                    header('location:commentaires.php?msg=rnu');
                    exit;
                }
        }
