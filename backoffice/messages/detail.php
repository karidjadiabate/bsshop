...
if (isset($_REQUEST['product_id']) && $_REQUEST['product_id'] != "") {
  ...
  $condition .= 'AND product_id="' . $_REQUEST['product_id'] . '"';
  $allMass = $db->getAllRecords('comment_by_product', '*', $condition, 'ORDER BY created_at DESC');
                ...
                <div class="card card-default">
                  ...
                  <div align="center">
                    <h3><b><?php echo count($allMass);
                            ?> commentaire(s) sur ce produit</b></h3>
                  </div>
                  <!-- Table Product -->
                  <div class="row">
                    <div class="col-12">
                              ...
                              if (count($allMass) > 0) {
                                foreach ($allMass as $val) {
                                  ...
                                  <tr>
                                    ...
                                    <td><?php echo $val['prenomClient'] . " " . $val['nomClient'] ?></td>
                                    <td><?php echo $val['created_at'] ?></td>
                                    <td>
                                      <?php
                                      $address = explode(' ', $val['commentaires']);
                                      $ff = 0;
                                ...
                                <tr>
                                  <td colspan="6"><b>Aucun commentaire trouvé</b></td>
                                </tr>
                              <?php } ?>
                            </tbody>
                          </table>
    ...
    <script>
      ...
      function supprimer(id) {
        swal({
          ...
          .then((willDelete) => {
            if (willDelete) {
              window.location.href = "supprimer.php?delId=" + id + "&sec=DELMASSE";
            } else {
              swal({
                title: "Alerte !",
                text: "Operation de suppression non confirmée!",
