...
define('MENU', 'COMMENTAIRES');
$condition = '';
$condition .= 'AND statut="1"';
$allMass = $db->getAllRecords('products', '*', $condition, 'ORDER BY name ASC');
          ...
          <div class="row">
            <div class="col-xl-12">
              <?php if (isset($_REQUEST['msg']) and $_REQUEST['msg'] == "rds") {
                echo '<script type="text/javascript">swal({ title: "Accepté !", text: "commentaire supprimé avec succès!", icon: "success", confirmButtonText: "OK" });</script>';
              } elseif (isset($_REQUEST['msg']) and $_REQUEST['msg'] == "rus") {
                echo '<script type="text/javascript">swal({ title: "Accepté !", text: "commentaire modifié avec succès!", icon: "success", confirmButtonText: "OK" });</script>';
                ...
                echo '<script type="text/javascript">swal({ title: "Accepté !", text: "commentaire ajouté avec succès!", icon: "success", confirmButtonText: "OK" });</script>';
              } elseif (isset($_REQUEST['msg']) and $_REQUEST['msg'] == "rnad") {
                echo '<script type="text/javascript">swal({ title: "Refusé !", text: "Erreur d\'commentaire. Prière de réessayer!", icon: "error", confirmButtonText: "OK" });</script>';
              } elseif (isset($_REQUEST['msg']) and $_REQUEST['msg'] == "reinitialiser") {
                echo '<script type="text/javascript">swal({ title: "Accepté !", text: "commentaire réinitialisé avec succès!", icon: "success", confirmButtonText: "OK" });</script>';
              } elseif (isset($_REQUEST['msg']) and $_REQUEST['msg'] == "exist") {
                echo '<script type="text/javascript">swal({ title: "Erreur !", text: "Ajout Impossible,Cet commentaire Existe Deja !", icon: "error", confirmButtonText: "OK" });</script>';
                          ...
                          <tbody class="text-center">
                            ...
                            if (count($allMass) > 0) {
                              foreach ($allMass as $val) {
                                $condition = "AND statut='1' AND product_id='" . $val["id"] . "'";
                                $countComment = $db->getQueryCountNb('comment_by_product', '*', $condition);
                                $count++; ?>
                                <tr>
                                  <td>
                                    <span class="custom-checkbox">
                                  ...
                                  <td>
                                    ...
                                    <a class="btn btn-primary btn-xl" title="voir les commentaires sur ce produit"href="detail.php?product_id=<?php echo $val['id'] ?>"><i class="mdi mdi-eye"></i></a>
                                  </td>
                                </tr>
                              <?php }
                            } else { ?>
          ...
          .then((willDelete) => {
            if (willDelete) {
              ...
              $.ajax({
                ...
                dataType: 'JSON',
                data: {
                  submit: 'delete_multipleM',
                  id: selected_values
                },
                success: function(data) {
                  if (data['result'] == "deleteAllOk") {
                    window.location.href = "commentaires.php?msg=rds";
                  }
                }
              });
            } else {
