...
if (isset($_REQUEST['start']) and $_REQUEST['start'] != "" and isset($_REQUEST['end']) and $_REQUEST['end'] != "") {
  $condition .= 'AND created_at BETWEEN "' . $_REQUEST['start'] . ' 00:00:00" AND "' . $_REQUEST['end'] . ' 23:59:59"';
} else {
  $condition .= 'AND created_at BETWEEN "' . date('Y-m-01') . ' 00:00:00" AND "' . date('Y-m-' . $nb_jour) . ' 23:59:59"';
}
...
$allStock = $db->getAllRecords('histo_stock', '*', $condition, 'ORDER BY id ASC');
$resumeStock = $db->getAllRecords('stock', 'product_id,SUM(en_stock) as stock ,SUM(disponible) as disponible,statut', 'AND statut="1"', 'GROUP BY product_id ORDER BY id ASC');

$condition = 'AND statut="1"';
$allProducts = $db->getAllRecords('products', '*', $condition, 'ORDER BY name ASC');

// var_dump($resumeStock);exit;
$error = '';
...
if (isset($_REQUEST['valider']) and $_REQUEST['valider'] != "") {
  ...
  if (count($allStocks) === 1) {
    $oldStock = $allStocks[0]['en_stock'];
    $oldDispo = $allStocks[0]['disponible'];
    $newStock = $stock + $oldStock;
    $newDispo = $stock + $oldDispo;
    $data   =   array(
      // 'product_id' => $product,
      'disponible' => $newDispo,
      'en_stock' => $newStock,
    ...
    $insert =   $db->update('stock', $data, array('product_id' => $product));
    $data   =   array(
      'product_id' => $product,
      'en_stock' => $stock,
      'disponible' => $stock,
      'date_appro' => $date,
      'updated_at' => $_SESSION['userYopci']['nom'] . "#" . $_SESSION['userYopci']['iduser'],
    );
    $insert2 =   $db->insert('histo_stock', $data);
    if ($insert2) {
...
//     'updated_at' =>  gmdate("Y-m-d H:i:s"),
//   );
//   $insert =   $db->update('stock', $data, array('id' => $stockId));
//   if ($insert) {
//     header('location:index.php?msg=rus');
//     exit;
//   } else {
...
<head>
  ...
  <script type="text/javascript" src="../css/select2.min.css"></script>
      ...
      <div class="content-wrapper">
        <div class="content">
          <div class="row">
            <div class="col-xl-12">
              ...
              <div class="card card-default">
                <div class="card">
                  ...
                  <form class="mt-5 ml-4 mr-4" method="GET">
                    <h3 class="mb-5">Rechercher </h3>
                    <div class="form-row">
                      <div class="col-4">
                        <select name="product" id="product" class="form-control">

                          <option value=""> PRODUIT(SELECTIONNEZ UN PRODUIT)</option>
                          <?php foreach ($allProducts as $key) { ?>
                            <option value="<?php echo $key['id'] ?>"><?php echo $key['name'] ?></option>
                          <?php } ?>
                        </select>
                      </div>
                      <span class="mt-2"><b>PERIODE DU</b></span>
                      <div class="col-3 ">
                      ...
                      <div class="col-12 mt-3">
                        ...
                        <a href="<?php echo PATH ?>stocks/index.php" class="btn btn-danger btn-sm ">Reinitialiser</a>
                    ...
                    <div class="col-12">
                      <div class="card card-default">
                        <div class="card-body">
                          ...
                          <button class="btn btn-success  btn-sm text-white col-md-3 float-right" style="font-size:11px"id="resume">Résumé global du stock</button>
                              ...
                              if (count($allStock) > 0) {
                                foreach ($allStock as $val) {
                                  ...
                                  <tr>
                                    ...
                                    <td>
                                      <?php foreach ($allProducts as $key) {
                                        if ($key['id'] == $val['product_id']) {
                                          echo $key['name'];
                                        }
                                      } ?></td>
                                    <td><?php echo $val['en_stock'] ?></td>
                                    <!-- <td><?php //echo $val['disponible'] 
                                              ?></td> -->

                                    <td><?php echo $val['date_appro'] ?></td>
                                    <td>
                                      ...
                                      <!-- <a class="btn btn-warning btn-sm" href="#" onClick="update('<?php echo $val['id']; ?>')" data-toggle="modal" data-target="#modal-stockU">Modifier</a> -->
                                ...
                                <tr>
                                  <td colspan="7"><b>Aucun enregistrement trouvé</b></td>
                                </tr>
                              <?php } ?>
                            </tbody>
                          </table>
                ...
                <form action="" method="POST" enctype="multipart/form-data">
                  <div class="modal-content">
                    <div class="modal-header align-items-center ">
                      <h2 class="modal-title" id="exampleModalGridTitle">Ajouter un Stock</h2>
                      <div>
                      ...
                      <div class="row">
                        <div class="col-lg-12">
                          ...
                          <div class="form-group mb-5">
                            ...
                            <select name="product" id="product" class="produits form-control">
                          ...
                          <div class="form-group mb-5">
                            <label for="new-product">Quantité à approvisionner<span class="text-danger">*</span></label>
                            <input type="number" min="10" class="form-control" required id="stock" name='stock' placeholder="Quantité">
                          </div>
                          <div class="form-group mb-5">
                            <label for="new-product">Date d'approvisionnement<span class="text-danger">*</span></label>
            ...
            <!-- Update Stock Modal -->
            <div class="modal fade modal-stockU" id="modal-stockU" aria-labelledby="modal-stockU" aria-hidden="true">
              <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
                <form action="" method="POST" enctype="multipart/form-data">
                  <div class="modal-content">
                    <div class="modal-header align-items-center ">
                      <h2 class="modal-title" id="exampleModalGridTitle">Modifier un Stock</h2>
                      <div>
                        <button type="button" class="btn btn-light btn-pill mr-1 mr-md-2" data-dismiss="modal"> Retour </button>
                    ...
                    <div class="modal-body ">
                      <div class="row">
                        <div class="col-lg-12">
                          ...
                          <div class="form-group mb-5">
                            <label for="new-product">Libellé de l'article<span class="text-danger">*</span></label>
                            <select disabled name="product" id="productU" class="form-control">
                              <option>SELECTIONNER UN PRODUIT</option>
                              <?php foreach ($allProducts as $key) { ?>
                                <option value="<?php echo $key['id'] ?>"><?php echo $key['name'] ?></option>
                              <?php } ?>
                          ...
                          <div class="form-group mb-5">
                            <label for="new-product">Quantité à approvisionner<span class="text-danger">*</span></label>
                            <input type="number" min="10" class="form-control" required id="stockU" name='stock' placeholder="Quantité">
                          </div>
                          <div class="form-group mb-5">
                            <label for="new-product">Date d'approvisionnement<span class="text-danger">*</span></label>
                            <input type="datetime-local" class="form-control" required id="dateU" name='date'>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <!-- Resume du Stock Modal -->
            <div class="modal fade modal-stock-resume" id="modal-stock-resume" aria-labelledby="modal-stock-resume" aria-hidden="true">
              <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
                <form action="" method="POST" enctype="multipart/form-data">
                  <div class="modal-content">
                    <div class="modal-header align-items-center ">
                      <h2 class="modal-title" id="exampleModalGridTitle">RESUME DU STOCK A CE JOUR</h2>
                      <div>
                        <button type="button" class="btn btn-light btn-pill mr-1 mr-md-2" data-dismiss="modal"> Retour </button>
                        <!-- <button type="submit" class="btn btn-primary  btn-pill" name="valider"  value="valider"> Valider </button> -->
                      </div>
                    ...
                    <div class="modal-body ">
                      <div class="row">
                        <div class="col-md-12">
                          ...
                          <!-- <h3 class="h5 mb-5">les champs contenant <span class="text-danger">*</span> sont obligatoires</h3> -->
                          <div class="form-group mb-5">
                            <table class="display compact cell-border nowrap" cellspacing="0" width="100%" id='secondtable' class="table table-hover table-product" style="width:100%">
                              <thead class="text-center">
                                <tr>
                                  <th>#</th>
                                  <th>Produit</th>
                              ...
                              <tbody class="text-center">
                                <?php $count = '';
                                if (count($resumeStock) > 0) {
                                  foreach ($resumeStock as $val) {
                                    $count++; ?>
                                    <tr>
                                      <td>
                                        <?php echo $count ?>
                                      ...
                                      <td><?php echo $val['disponible'] ?></td>
                                      <td><?php if ($val['disponible'] > 0) {
    ...
    <script>
      ...
      function supprimer(id) {
        swal({
          ...
          .then((willDelete) => {
            if (willDelete) {
              window.location.href = "supprimer.php?delId=" + id + "&sec=STOCK";
            } else {
              swal({
                title: "Alerte !",
                text: "Operation de suppression non confirmée!",
      ...
      function update(id) {
        $.ajax({
          ...
          success: function(data) {
            console.log(data);
            $("#productU").val(data['product_id']);
            $("#disponible").val(data['disponible']);
            $("#stockU").val(data['en_stock']);
            $("#dateU").val(data['date_appro']);
            $("#stockId").val(id);
          }
        });
      }
